...
from agent import create_budget_agent  # Import the factory function
...
class BudgetAgentExecutor(AgentExecutor):
    """
    AgentExecutor for the Travel Budget ADK Agent.
    ...
    def __init__(self, runner: Runner, card: AgentCard):
        ...
        self.adk_agent = create_budget_agent()
    ...
    async def _process_request(
        ...
        try:
            ...
            logger.exception("Error during budget agent execution")
            await task_updater.update_status(
                TaskState.failed,
                message=task_updater.new_agent_message(
    ...
    async def execute(
        ...
        logger.debug("[BudgetAgent] execute completed")
    ...
    async def cancel(
        ...
        """Attempt to cancel ongoing budget agent session."""
        session_id = context.context_id

        if session_id in self._active_sessions:
            logger.info(f"Cancellation requested for active budget session: {session_id}")
